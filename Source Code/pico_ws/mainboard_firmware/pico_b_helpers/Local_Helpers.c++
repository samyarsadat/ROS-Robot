...
bool check_bool(bool function, RT_CHECK_MODE mode)
{
    if (!function)
    {
        ...
        {
            case RT_HARD_CHECK:
                publish_diag_report(DIAG_LVL_ERROR, DIAG_HWNAME_UCONTROLLERS, DIAG_HWID_MCU_MABO_B, DIAG_ERR_MSG_BOOL_RT_CHECK_FAIL, NULL);
                clean_shutdown();
                break;

            case RT_SOFT_CHECK:
                publish_diag_report(DIAG_LVL_WARN, DIAG_HWNAME_UCONTROLLERS, DIAG_HWID_MCU_MABO_B, DIAG_WARN_MSG_BOOL_RT_CHECK_FAIL, NULL);
                break;
