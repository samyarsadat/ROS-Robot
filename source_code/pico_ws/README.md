...
Please take a look at the [`pico-build` workflow](https://github.com/samyarsadat/ROS-Robot/actions/workflows/pico-build.yml) artifact called `pico_binary_artifacts` for the latest binary builds.

#### Action Status
[![Build Pico Source Code](https://github.com/samyarsadat/ROS-Robot/actions/workflows/pico-build.yml/badge.svg)](https://github.com/samyarsadat/ROS-Robot/actions/workflows/pico-build.yml)\
...
When re-building the devcontainer, the re-build might fail the first time.\
In this case, simply retry the build a second time.

You might have to wait (even after the devcontainer has started) for the MicroROS tools to install.\
Reload the devcontainer after you have verified that the MicroROS tools have installed successfully.\
(Check whether `~/pico_ws/libmicroros/src/micro_ros_setup/install` exists.)
